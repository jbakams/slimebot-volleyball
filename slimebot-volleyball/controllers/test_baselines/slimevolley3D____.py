...
class Particle:
  ...
  def applyAcceleration(self, ax, ay, az):
    self.vx += ax * TIMESTEP
    self.vy += ay * TIMESTEP
    self.vz += az * TIMESTEP * (self.world.depth/self.world.max_depth)
    
    
    
  def checkEdges(self):
    if (self.x <= (self.r-self.world.width/2)):
      self.vx *= -FRICTION
      self.x = self.r-self.world.width/2+NUDGE*TIMESTEP

    if (self.x >= (self.world.width/2-self.r)):
      self.vx *= -FRICTION;
      self.x = self.world.width/2-self.r-NUDGE*TIMESTEP
      
    if self.world.depth >= self.r:
      
        if (self.z<=(self.r-self.world.depth/2)):
          self.vz *= -FRICTION * (self.world.depth/self.world.max_depth)
          self.z = self.r-self.world.depth/2+NUDGE*TIMESTEP
          
        if (self.z >= (self.world.depth/2-self.r)):
          self.vz *= -FRICTION * (self.world.depth/self.world.max_depth);
          self.z = self.world.depth/2-self.r-NUDGE*TIMESTEP
          
    else:
        if (self.z<=(self.world.depth/2)):
          self.vz *= -FRICTION * (self.world.depth/self.world.max_depth)
          self.z =-self.world.depth/2+NUDGE*TIMESTEP
    ...
    if (self.y >= (self.world.height-self.r)):
      self.vy *= -FRICTION
      self.y = self.world.height-self.r-NUDGE*TIMESTEP
      
    # fence:
    if ((self.x <= (self.world.wall_width/2+self.r)) and (self.prev_x > (self.world.wall_width/2+self.r)) and (self.y <= self.world.wall_height)):
      self.vx *= -FRICTION
      self.x = self.world.wall_width/2+self.r+NUDGE*TIMESTEP
  ...
  def isColliding(self, p): # returns true if it is colliding w/ p
    r = self.r+p.r
    if self.world.depth != 0:
      return (r*r > self.getDist2(p) and (self.z*self.z <= self.world.wall_depth * self.world.wall_depth)) # if distance is less than total radius and the depth, then colliding.
    else:
  ...
  def limitSpeed(self, minSpeed, maxSpeed):
    ...
    if (mag2 > (maxSpeed*maxSpeed) ):
      ...
      self.vx *= maxSpeed
      self.vy *= maxSpeed
      self.vz *= maxSpeed * (self.world.depth/self.world.max_depth)
    ...
    if (mag2 < (minSpeed*minSpeed) ):
      ...
      self.vx *= minSpeed
      self.vy *= minSpeed
      self.vz *= minSpeed * (self.world.depth/self.world.max_depth)
...
class Agent():
  ...
  def setAction(self, action):
    ...
    if (forward and (not backward)):
      self.desired_vx = -self.world.player_vx
    if (backward and (not forward)):
      self.desired_vx = self.world.player_vx
    if jump:
      self.desired_vy = self.world.player_vy
    if (right and (not left)):
      self.desired_vz = self.world.player_vz
  ...
  def update(self):
    ...
    if (self.x*self.dir <= (self.world.wall_width/2+self.r) ):
      self.vx = 0;
      self.x = self.dir*(self.world.wall_width/2+self.r)

    if (self.x*self.dir >= (self.world.width/2-self.r) ):
      self.vx = 0;
      self.x = self.dir*(self.world.width/2-self.r)
      
    # stay in scene area:
    
    if self.world.wall_depth >= self.r:
      if (self.z <= -(self.world.wall_depth/2 - self.r) ):
        self.vz = 0;
        self.z = -(self.world.wall_depth/2-self.r)
    
      if (self.z >= self.world.wall_depth/2 - self.r ):
        self.vz = 0;
        self.z = self.world.wall_depth/2 - self.r
    else:
      if (self.z <= -(self.world.wall_depth/2) ):
        self.vz = 0;
        self.z = -(self.world.wall_depth/2)
...
class Game:
  ...
  def reset(self):    
    ...
    self.fenceStub = Particle(0, self.world.wall_height, 0, 0, 0, 0, self.world.wall_width/2, "FENCESTUB");
    if self.training:
    ...
    ball_vz = self.np_random.uniform(low=-10, high=10) * (self.world.depth/self.world.max_depth)
    self.ball = Particle(0, (self.world.width/4)-1.5, 0, ball_vx, ball_vy, ball_vz, 0.5, "BALL");
    self.agent_left = Agent(-1, -self.world.width/4, 1.5, 0, "BLUE")
    self.agent_right = Agent(1, self.world.width/4, 1.5, 0, "YELLOW")
  ...
  def step(self):
    ...
    if self.delayScreen.status():
      self.ball.applyAcceleration(0, self.world.gravity, 0)
      self.ball.limitSpeed(0, self.world.max_ball_v)
      self.ball.move()
...
class Slime3DEnv(gym.Env):
  ...
  def __init__(self, training = False, upgrade = False, num_upgr = 0, step_upgr = 0):
    ...
    self.world_max_depth = WORLD.max_depth
